 ...
 *
 * PERUBAHAN: $ZOOM_OPTIONS dan $UNIT_OPTIONS kini diambil dari DB
 *            (tabel zoom_links dan zoom_units) agar bisa dikelola
 *            via halaman Master Zoom di Administrator.
 */
...
// ── Ambil dari DB (hanya yang is_active = 1) ─────────────
$ZOOM_OPTIONS = $pdo->query(
    "SELECT email FROM zoom_links WHERE is_active = 1 ORDER BY sort_order ASC, id ASC"
)->fetchAll(PDO::FETCH_COLUMN);

$UNIT_OPTIONS = $pdo->query(
    "SELECT name FROM zoom_units WHERE is_active = 1 ORDER BY sort_order ASC, id ASC"
)->fetchAll(PDO::FETCH_COLUMN);

// Fallback jika tabel belum ada / kosong — pakai array hardcode lama
// supaya form tidak langsung kosong sebelum migration dijalankan.
if (empty($ZOOM_OPTIONS)) {
    $ZOOM_OPTIONS = [
...
if (empty($UNIT_OPTIONS)) {
    $UNIT_OPTIONS = [
        ...
        'NIAGA', 'KEUANGAN', 'DISTRIBUSI', 'UP2K', 'SDM', 'YBM', 'IKPLN',
        'UID Jawa Tengah & D.I. Yogyakarta',
        'UP3 Kudus', 'UP3 Surakarta', 'UP3 Yogyakarta', 'UP3 Magelang',
        'UP3 Purwokerto', 'UP3 Tegal', 'UP3 Semarang', 'UP3 Salatiga',
        'UP3 Klaten', 'UP3 Pekalongan', 'UP3 Cilacap', 'UP3 Grobogan',
        'UP3 Sukoharjo', 'UP2D Jateng & DIY',
    ];
}

// ── Zoom yang sedang DIPAKAI ──────────────────────────────
$stmt_busy = $pdo->query("
...
if ($_SERVER['REQUEST_METHOD'] === 'POST'
    ...
    if (!$errors) {
        ...
        $booking_time = date('H:i', strtotime($start_datetime))
                      . ' - '
                      . date('H:i', strtotime($end_datetime));
