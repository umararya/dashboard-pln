...
// Ikon & label per tab
$TAB_META = [
    'nama'   => ['icon' => '🖥️',  'label' => 'Nama Perangkat',   'placeholder' => 'Contoh: Aplikasi Web, ERP, SCADA'],
...
.mpa-add-form input[type="text"]:focus {
    border-color: #22c55e;
...
.mpa-modal {
    ...
    background: rgba(0,0,0,.48); z-index: 3000;
...
.mpa-modal-footer {
    ...
    padding: 16px 22px; border-top: 1px solid #e5e7eb; background: #f8fafc;
}
...
<!-- Edit Modal (shared, diisi via JS) -->
...
<div class="mpa-card" style="margin-bottom:22px;">
    ...
    <div style="padding:0 0 4px;">
        <?php foreach ($TAB_META as $key => $meta): ?>
            <div class="mpa-panel <?= $active_tab === $key ? 'active' : '' ?>" id="panel-<?= $key ?>">
                    ...
                    <table class="mpa-table">
                        ...
                        <tbody>
                            <?php if (empty($data[$key])): ?>
                                <tr>
                                    <td colspan="4" class="mpa-empty">
                                        Belum ada data <?= h($meta['label']) ?>
                                    </td>
                                </tr>
                            <?php else: ?>
                                <?php $no = 1; foreach ($data[$key] as $item): ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="mpa-btn-group">
                                                ...
                                                <button
                                                    type="button"
                                                    class="btn-mpa btn-mpa-edit"
                                                    onclick='openMpaEdit("<?= $key ?>", <?= (int)$item['id'] ?>, <?= json_encode($item['name']) ?>, "<?= h($meta['label']) ?>")'
                                                >✏️</button>
                                                ...
                                                <form method="post" style="margin:0;"
                                                    ...
                                                    <input type="hidden" name="tab_key"  value="<?= $key ?>">
                                                    <input type="hidden" name="item_id"  value="<?= (int)$item['id'] ?>">
                                                    <button type="submit" class="btn-mpa btn-mpa-del" title="Hapus">🗑️</button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
                ...
                <div class="mpa-footer-note">
                    💡 Item dengan status <strong>Nonaktif</strong> tidak akan muncul di dropdown form Input &amp; Edit Perangkat Aplikasi.
                </div>
            </div><!-- /panel -->
        <?php endforeach; ?>
    </div>
...
<script>
/* ── Tab switching (handled via URL, tapi tambahkan smooth UX) ── */
// Tab sudah dihandle via server-side (href URL), tidak perlu JS khusus.
...
function closeMpaModal() {
    document.getElementById('mpaEditModal').classList.remove('show');
}

/* Tutup saat klik backdrop */
document.addEventListener('click', function (e) {
    if (e.target && e.target.id === 'mpaEditModal') {
        closeMpaModal();
    }
});
